...
export default class TradeCommand extends Command {
	...
	private async tradeConfirmation(interaction: CommandInteraction, meta: MetaData) {
		const row = new MessageActionRow().addComponents(
			new MessageButton().setCustomId('yes').setLabel('Confirm Trade').setStyle('SUCCESS'),
			new MessageButton().setCustomId('no').setLabel('Cancle Trade').setStyle('DANGER')
		);
				...
				case 'yes':
					{
						...
						await this.progress(interaction, meta);
					}
					break;
				case 'no':
					{
						await i.update({ content: 'Trade successfully canceled.', components: [] });
					}
					break;
			}
		});
		offerConfirmation.on('end', () => {
			offerConfirmationReply.components = [];
		});
	}

	private async progress(interaction: CommandInteraction, meta: MetaData) {
		const row = new MessageActionRow().addComponents(
				...
				case 'cancel':
					{
						const canceledEmbed = new MessageEmbed()
							...
							.setDescription(
								`A trade between ${meta.offer.from.user.toString()} and ${
									meta.requested.from.user
								} was canceled by ${i.user.toString()}.`
							);
						await followUp?.edit({ components: [], embeds: [canceledEmbed] });
					}
					break;
				case 'lock':
					{
						if (!meta.lockedIn) {
							...
							await i.reply({ content: 'You are now locked in!', ephemeral: true });
							await followUp?.edit({
								embeds: [
